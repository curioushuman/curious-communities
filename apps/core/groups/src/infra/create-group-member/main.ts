...
export const handler = async (
  requestDtoOrEvent:
    ...
    | EventBridgeEvent<'putEvent', CreateGroupMemberRequestDto>
): Promise<void> => {
  // grab the dto
  const requestDto =
  ...
  const logger = new LoggableLogger('CreateGroupMemberFunction.handler');
  logger.debug ? logger.debug(requestDto) : logger.log(requestDto);

  // lambda level validation
  if (!CreateGroupMemberRequestDto.guard(requestDto)) {
    // NOTE: this is a 500 error, not a 400
    const error = new InternalRequestInvalidError(
      'Invalid request sent to CreateGroupMemberFunction.Lambda'
    );
  ...
  const createGroupMemberController = app.get(CreateGroupMemberController);
  ...
  return createGroupMemberController.create(requestDto);
};
