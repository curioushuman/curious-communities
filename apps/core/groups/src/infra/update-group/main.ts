...
export const handler = async (
  requestDtoOrEvent:
    ...
    | EventBridgeEvent<'putEvent', UpdateGroupRequestDto>
): Promise<void> => {
  // grab the dto
  const requestDto =
  ...
  if (!UpdateGroupRequestDto.guard(requestDto)) {
  ...
  const updateGroupController = app.get(UpdateGroupController);
  ...
  return updateGroupController.update(requestDto);
};
