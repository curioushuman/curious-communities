...
import {
  locateDto,
  UpsertCourseGroupMemberDtoOrEvent,
...
async function bootstrap() {
  ...
  UpsertCourseGroupMemberModule.applyDefaults(app);
  return app;
}
...
export const handler = async (
  requestDtoOrEvent: UpsertCourseGroupMemberDtoOrEvent
): Promise<CourseGroupMemberBaseResponseDto | void> => {
  // grab the dto
  ...
  const logger = new LoggableLogger('UpsertCourseGroupMemberFunction.handler');
  logger.debug ? logger.debug(requestDto) : logger.log(requestDto);

  // lambda level validation
  if (!requestDto || !UpsertCourseGroupMemberRequestDto.guard(requestDto)) {
    // NOTE: this is a 500 error, not a 400
    const error = new InternalRequestInvalidError(
      'Invalid request sent to UpsertCourseGroupMemberFunction.Lambda'
    );
  ...
  const controller = app.get(UpsertCourseGroupMemberController);

  // try/catch doesn't work at this level
  return controller.upsert({
    participant: requestDto.participant,
  });
};
