...
export const handler = async (
...
): Promise<CourseGroupBaseResponseDto | void> => {
  // grab the dto
  ...
  const logger = new LoggableLogger('UpsertCourseGroupFunction.handler');
  logger.debug ? logger.debug(requestDto) : logger.log(requestDto);

  // lambda level validation
  if (!requestDto || !UpsertCourseGroupRequestDto.guard(requestDto)) {
    // NOTE: this is a 500 error, not a 400
    const error = new InternalRequestInvalidError(
      'Invalid request sent to UpsertCourseGroupFunction.Lambda'
    );
