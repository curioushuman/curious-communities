...
/**
 * Components required for the api-admin stack competitions:hook resource
 *
 * TODO:
...
export class HookConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: HookProps) {
     ...
     */
    const eventSourceId = `apigw-${this.apiConstruct.id}-competitions-hook`;
    const competitionsHookRequestTemplate = CoApiConstruct.vtlTemplateFromFile(
    ...
    // SUCCESS
    const competitionsHookFunctionSuccessResponse: apigateway.IntegrationResponse =
      {
        statusCode: '200',
        responseTemplates: {
    ...
    // ERROR
    const competitionsHookFunctionServerErrorResponse =
      CoApiConstruct.serverErrorResponse();
    const competitionsHookFunctionClientErrorResponse =
    ...
    this.awsIntegration = new apigateway.AwsIntegration({
      ...
      options: {
        ...
        requestTemplates: {
          'application/json': competitionsHookRequestTemplate,
        },
        passthroughBehavior: apigateway.PassthroughBehavior.NEVER,
        integrationResponses: [
          competitionsHookFunctionSuccessResponse,
          competitionsHookFunctionServerErrorResponse,
          competitionsHookFunctionClientErrorResponse,
        ],
      },
    });
