...
import {
  CreateCompetitionModule,
  CreateCompetitionController,
} from '@curioushuman/ue-competitions-service';
import { InternalRequestInvalidError } from '@curioushuman/error-factory';
import { LoggableLogger } from '@curioushuman/loggable';

import { CreateCompetitionRequestDto } from './dto/request.dto';
...
export const handler = async (
  requestDtoOrEvent:
    ...
    | EventBridgeEvent<'putEvent', CreateCompetitionRequestDto>
): Promise<void> => {
  // grab the dto
  const requestDto =
  ...
  const logger = new LoggableLogger('CreateCompetitionFunction.handler');
  logger.debug ? logger.debug(requestDto) : logger.log(requestDto);

  // lambda level validation
  if (!CreateCompetitionRequestDto.guard(requestDto)) {
    // NOTE: this is a 500 error, not a 400
    const error = new InternalRequestInvalidError(
      'Invalid request sent to CreateCompetitionFunction.Lambda'
    );
  ...
  const createCompetitionController = app.get(CreateCompetitionController);
  ...
  return createCompetitionController.create(requestDto);
};
