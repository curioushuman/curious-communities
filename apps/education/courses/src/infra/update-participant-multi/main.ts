...
import {
  UpdateParticipantMultiModule,
  UpdateParticipantMultiController,
...
import {
  locateDto,
  UpdateParticipantMultiDtoOrEvent,
...
async function bootstrap() {
  ...
  UpdateParticipantMultiModule.applyDefaults(app);
  return app;
}
...
export const handler = async (
  requestDtoOrEvent: UpdateParticipantMultiDtoOrEvent
): Promise<void> => {
  const context = 'UpdateParticipantMulti.Lambda';
  ...
  const validRequestDto = validateRequestPayload({
    requestPayload,
    checkRequest: UpdateParticipantMultiRequestDto.guard,
    logger,
  });

  // init the app
  const app = await waitForApp();
  const controller = app.get(UpdateParticipantMultiController);
