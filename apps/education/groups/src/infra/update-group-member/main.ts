...
import { UpdateGroupMemberRequestDto } from './dto/request.dto';
...
export const handler = async (
  requestDtoOrEvent:
    ...
    | EventBridgeEvent<'putEvent', UpdateGroupMemberRequestDto>
): Promise<void> => {
  // grab the dto
  const requestDto =
  ...
  if (!UpdateGroupMemberRequestDto.guard(requestDto)) {
  ...
  const updateGroupMemberController = app.get(UpdateGroupMemberController);
  ...
  return updateGroupMemberController.update(requestDto);
};
