...
describe('ApiAdminStack : hook', () => {
  ...
  describe('/competitions/{eventType}/{externalId}?{updatedStatus?}', () => {
    const resourceParentRegex =
      'ueapiadminRestApicompetitionshookeventType[A-Z0-9]+';
    const resourceIdRegex =
      'ueapiadminRestApicompetitionshookeventTypeexternalId[A-Z0-9]+';
    ...
    describe('GET', () => {
      ...
      describe('With four method responses', () => {
        ...
        test('Success returning the competition response DTO model', () => {
          const responses = methodResponses.asArray();
          const successResponse = responses.find(
            (response) => response.StatusCode === '200'
          );
