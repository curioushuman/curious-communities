...
import { CreateCourseGroupMemberMapper } from '../../application/commands/create-course-group-member/create-course-group-member.mapper';
import { CreateCourseGroupMemberCommand } from '../../application/commands/create-course-group-member/create-course-group-member.command';
...
/**
 * Controller for create course-group-member operations
 *
 * TODO
 * - [ ] switch to upsert
 */
...
@Controller()
export class CreateCourseGroupMemberController {
  constructor(
    private logger: LoggableLogger,
    private readonly commandBus: CommandBus,
    private readonly queryBus: QueryBus
  ) {
    this.logger.setContext(CreateCourseGroupMemberController.name);
  }
  ...
  public async create(
    ...
    const task = pipe(
      ...
      // #2. transform the dto
      parseActionData(
        CreateCourseGroupMemberMapper.fromRequestDto(group),
        this.logger
      ),
      ...
      TE.chain((commandDto) =>
        TE.tryCatch(
          async () => {
            const command = new CreateCourseGroupMemberCommand(commandDto);
            return await this.commandBus.execute<CreateCourseGroupMemberCommand>(
              command
            );
          },
