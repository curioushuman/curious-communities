...
Scenario: Fail; Source is already associated with a GroupMember
  Given the request is valid
  And I am authorised to access the source
  And a matching record is found at the source
  And the returned source populates a valid group-member
  And the returned source is already associated with a GroupMember
  When I attempt to create a group-member
  Then I should receive a SourceInvalidError
  And no result is returned
