...
import { BettermodeApiHttpConfigService } from '../http-config.service';
...
defineFeature(feature, (test) => {
  let httpConfigService: BettermodeApiHttpConfigService;

  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      imports: [
        HttpModule.registerAsync({
          useClass: BettermodeApiHttpConfigService,
        }),
      ],
      providers: [
        LoggableLogger,
        {
          ...
          useClass: BettermodeApiRepositoryErrorFactory,
        },
      ],
    }).compile();

    httpConfigService = moduleRef.get<BettermodeApiHttpConfigService>(
      BettermodeApiHttpConfigService
    );
  });
